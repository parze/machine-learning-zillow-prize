...
\textit{Mean Absolute Error} is then used as the metric on the logerror individually for Baseline, LightGBM, XGBoost as well
...
% TODO: will be a hard problem to predict
...
Major parameters for both models where tuned through the steps of changing one paremeter at a time to different
values while reruning the prediction and see if the train and test data result improved. In case of
\begin{enumerate}
    ...
    \item train result and the test result improved indicates that the change made the model generalize better and
        the change was keept
\end{enumerate}

Following improvments were made per model.
\subsubsection{LightGBM}
\begin{itemize}
    \item Changed learning rate from default value of 0.1 to 0.0021 which mitigated overfittning
    \item Changed number of leaves in one tree from default value of 31 to 512 which improved result
\end{itemize}
\subsubsection{XGBoost}
\begin{itemize}
    \item Changed the step size shrinkage (eta) from default value of 0.3 to 0.03295 which mitigated overfittning
    \item Changed the maximum depth of a tree from default value of 6 to 8 which improved the model
        without overfittning it by increaseing complexity
    \item Changed the subsample ratio from default value of 1 to 0.8 which mitigated overfittning
\end{itemize}
...
% The final models qualities such as parameters are evaluated in detail. Some type of analysis is used
% to validate the robustness of the models solution.
...
traning as well test data.

Changing the \textit{random state seed} will pick a different train set, pick a different test set, and train models
using different entrophy. Following are the seeds and errors from the different models.
\begin{center}
\begin{tabular}{ l l l l l l l }
    \textbf{Seed} & \textbf{Zero} & \textbf{Baseline} & \textbf{LightGBM} & \textbf{XGBoost} & \textbf{Weighted} & \textbf{Diff} \\
...
\end{tabular}
\end{center}
The differens between Benchmark/Zero model and the Weighted model result seems to stay steady above
\textbf{0.000797}. This indicates that the Weighted model can find and use some information in the data to
make a better prediction than just a simple zero approche. That said, it is still a lot longer way down
...
Interesting observation is that the weighing the predictions the Baseline, LightGBM, and XGBoost generates a
better overall MAE then for eny individual model.
